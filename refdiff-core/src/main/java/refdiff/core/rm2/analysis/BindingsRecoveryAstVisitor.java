...
public class BindingsRecoveryAstVisitor extends ASTVisitor {
	...
	private final LinkedHashMap<String, String> methodBody;

	public BindingsRecoveryAstVisitor(SDModel.Snapshot model,
			String sourceFilePath, char[] fileContent, SDPackage sdPackage,
			Map<SDEntity, List<String>> postProcessReferences,
			Map<SDType, List<String>> postProcessSupertypes,
			Map<String, List<SourceRepresentation>> postProcessClientCode,
			SourceRepresentationBuilder srbForTypes,
			SourceRepresentationBuilder srbForMethods,
			SourceRepresentationBuilder srbForAttributes,
			Map<String, Map<Integer, Integer>> lineMethods,
			LinkedHashMap<String, String> methodBody) {
		this.model = model;
	...
	public boolean visit(AnonymousClassDeclaration node) {
		if (node.getParent() instanceof ClassInstanceCreation) {
			ClassInstanceCreation parent = (ClassInstanceCreation) node
					.getParent();
			ITypeBinding typeBinding = parent.getType().resolveBinding();
			if (typeBinding != null && typeBinding.isFromSource()) {
				SDType type = model.createAnonymousType(containerStack.peek(),
						sourceFilePath, "");
	...
	public boolean visit(EnumDeclaration node) {
		containerStack.push(visitTypeDeclaration(node,
	...
	private SDType visitTypeDeclaration(AbstractTypeDeclaration node,
			List<Type> supertypes) {
		SDType type;
		String typeName = node.getName().getIdentifier();
		if (node.isLocalTypeDeclaration()) {
			type = model.createAnonymousType(containerStack.peek(),
					sourceFilePath, typeName);
		} else {
			type = model.createType(typeName, containerStack.peek(),
					sourceFilePath);
		}
		// type.setSourceCode(srbForTypes.buildSourceRepresentation(fileContent,
		// node.getStartPosition(), node.getLength()));
		type.setSourceCode(srbForTypes.buildSourceRepresentation(type,
				fileContent, node));
	...
	private void extractSupertypesForPostProcessing(SDType type,
		...
		while (superTypeBinding != null && superTypeBinding.isFromSource()) {
			String superTypeName = superTypeBinding.getErasure()
					.getQualifiedName();
	...
	public boolean visit(MethodDeclaration methodDeclaration) {
		...
		// return false;
		// }
		String methodSignature = AstUtils
				.getSignatureFromMethodDeclaration(methodDeclaration);
		// if
		...
		this.lineMethods.put(methodDeclaration.getName().toString() + "@@"
				+ methodDeclaration.parameters().toString(), innerMap);
		
		final SDMethod method = model.createMethod(methodSignature,
				containerStack.peek(), methodDeclaration.isConstructor());
		...
		method.setDeprecatedAnnotation(annotations.contains("Deprecated")
				|| AstUtils.containsDeprecatedTag(methodDeclaration
						.getJavadoc()));
		...
		method.setNumberOfStatements(AstUtils
				.countNumberOfStatements(methodDeclaration));
		Block body = methodDeclaration.getBody();
		if (body == null) {
			method.setSourceCode(srbForMethods.buildEmptySourceRepresentation());
			method.setAbstract(true);
			//if ((methodDeclaration.parameters().size() > 0)
				//	&& (methodDeclaration.getBody().getLength() > 0)) {
				//this.methodBody.put(methodDeclaration.getName().toString() + "@@"
					//	+ methodDeclaration.parameters().toString(),
					//	methodDeclaration.getBody().toString());
				//System.out.println("methodBody methodDeclaration"
					//	+ methodDeclaration.getModifiers());
			//}
		} else {
			// method.setSourceCode(srbForMethods.buildSourceRepresentation(this.fileContent,
			// body.getStartPosition() + 1, body.getLength() - 2));
			

			this.methodBody.put(sourceFilePath+"#"+methodDeclaration.getName().toString() + "@@"
					+ methodDeclaration.parameters().toString(),
					methodDeclaration.getBody().toString());

		//}
			method.setSourceCode(srbForMethods.buildSourceRepresentation(
					method, this.fileContent, body));

			final List<String> references = new ArrayList<String>();
			body.accept(new DependenciesAstVisitor(true) {
				@Override
				protected void onMethodAccess(ASTNode node,
						IMethodBinding binding) {
					String methodKey = AstUtils
							.getKeyFromMethodBinding(binding);
					references.add(methodKey);
				}

				@Override
				protected void onFieldAccess(ASTNode node,
						IVariableBinding binding) {
					String attributeKey = AstUtils
							.getKeyFromFieldBinding(binding);
					references.add(attributeKey);
					Statement stm = AstUtils.getEnclosingStatement(node);
					// if (stm == null) {
					// System.out.println("null");
					// }
					SourceRepresentation code = srbForAttributes
							.buildPartialSourceRepresentation(fileContent, stm);
	...
	private void addClientCode(String attributeKey, SourceRepresentation code) {
		List<SourceRepresentation> codeFragments = postProcessClientCode
				.get(attributeKey);
	...
	public boolean visit(FieldDeclaration fieldDeclaration) {
		...
		for (VariableDeclarationFragment fragment : fragments) {
			...
			final SDAttribute attribute = model.createAttribute(fieldName,
					containerStack.peek());
			attribute.setStatic(isStatic);
			attribute.setVisibility(visibility);
			attribute.setType(AstUtils.normalizeTypeName(fieldType,
					fragment.getExtraDimensions(), false));

			Expression expression = fragment.getInitializer();
			if (expression != null) {
				// attribute.setAssignment(srbForAttributes.buildSourceRepresentation(fileContent,
				// expression.getStartPosition(), expression.getLength()));
				addClientCode(attribute.key().toString(),
						srbForAttributes.buildPartialSourceRepresentation(
								fileContent, expression));
			}
			attribute.setClientCode(srbForAttributes
					.buildEmptySourceRepresentation());
		}
		return true;
	}
	...
	public static void extractParametersAndReturnType(
			MethodDeclaration methodDeclaration, SDMethod method) {
		Type returnType = methodDeclaration.getReturnType2();

		if (returnType != null) {
			method.setReturnType(AstUtils.normalizeTypeName(returnType,
					methodDeclaration.getExtraDimensions(), false));
		} else {
		...
		while (parameters.hasNext()) {
			...
			String typeName = AstUtils.normalizeTypeName(parameterType,
					parameter.getExtraDimensions(), parameter.isVarargs());
